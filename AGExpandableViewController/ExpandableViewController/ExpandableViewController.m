...
@property (nonatomic) CGFloat displayLinkPercent;
...
- (CGFloat)originHeight {
    if ([_childDatasource respondsToSelector:@selector(expandableViewControllerOriginHeight)]) {
        ...
        return _originHeight;//default origin state height
    }
}
...
- (CGFloat)halfHeight {
    if ([_childDatasource respondsToSelector:@selector(expandableViewControllerHalfHeight)]) {
        ...
        return _halfHeight;//default half state height
    }
}
...
- (CGFloat)entireHeightOffset {
    if ([_childDatasource respondsToSelector:@selector(expandableViewControllerEntireHeightOffset)]) {
        ...
        return _entireHeightOffset;//default entire state height
    }
}
...
- (void)startDisplayLink {
    ...
    self.startTime = CFAbsoluteTimeGetCurrent() + (1.0 - self.displayLinkPercent) * kDisplayLinkDuration;
...
- (void)handleDisplayLink:(CADisplayLink *)displayLink {
    ...
    self.displayLinkPercent = elapsed / kDisplayLinkDuration - floor(elapsed / kDisplayLinkDuration);
    self.view.transform = CGAffineTransformMakeRotation(M_PI * 2.0 * self.displayLinkPercent);
}

- (void)handleOriginDisplayLink:(CADisplayLink *)displatLink {
    self.view.transform = CGAffineTransformMakeRotation(M_PI * 2.0);
}
...
//    [self.view.superview addConstraint: [NSLayoutConstraint constraintWithItem:self.view attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view.superview attribute:NSLayoutAttributeBottom multiplier:0.9 constant:10]];
    [self.view.superview addConstraint: [NSLayoutConstraint constraintWithItem:self.view attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self.view.superview attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:0]];
    [self.view.superview addConstraint: [NSLayoutConstraint constraintWithItem:self.view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:[self superViewWidth]]];
    [self.view.superview addConstraint: [NSLayoutConstraint constraintWithItem:self.view attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:[self superViewHeight]]];
    [self.view.superview layoutIfNeeded];
}
